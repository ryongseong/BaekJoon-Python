...
if cur_rate>=99:    # 승률이 99 이상이라면 더 해도 오르지 않음.
    ...
    min = 1         # 1판 이상은 해야하므로 min의 초깃값 1로 할당
    max = game      # count가 게임 횟수보다 많을 수 없기에 max의 초깃값 game으로 할당
    while min <= max:           # min이 max보다 커질때 까지
        search = (min+max)//2   # 추가 게임 횟수를 찾기 위해 min+max의 정수형 평균을 할당.
        if (win+search) * 100 // (game+search) <= cur_rate: # 승률에 변화가 없다면
            min = search +1     # min 값을 추가 게임 횟수에 1을 더하여 할당.
        else:                   # 승률에 변화가 있다면
            count = search      # 그 게임의 수가 최소한 해야할 게임의 수가 됨.
            max = search-1      # while문을 탈출하기 위해 추가 게임의 수의 1을 뺀 값을 max에 할당
